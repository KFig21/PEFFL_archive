...
import { getHighestScoringGame } from "../../../helpers/apiCalls";

export default function HighestScoringGame({
  j_Division,
  helmetStyle,
  helmetView,
}) {
  ...
  const getGames = async () => {
    setGames(await getHighestScoringGame());
  };
  ...
  return (
    <div className="record-table" id="highest-scoring-game">
      <div className="table-name">
        <SC.textOnBgColor>Highest Scoring Game</SC.textOnBgColor>
      </div>
                ...
                return (
                  <SC.tableBorderColorTR
                    className={`records-team-row ${
                      (user.username === winner || user.username === loser) &&
                      user.highlightUser &&
                      "logged-in"
                    }`}
                    key={i}
